...
@section('print_card_body')
	...
	<table id="dataTable" name="paymentRecordTable" class="table table-bordered table-sm">
		...
		<tbody>
            ...
            @foreach ($prvPurchaseReturns as $prvPurchaseReturn)
                <tr>
                    <td>{{ $sl++ }}</td>
                    <td>{{ date('d-m-Y',strtotime($prvPurchaseReturn->date))}}</td>
                    <td>{{ $prvPurchaseReturn->invoice_no }}</td>
                    <td>{{ $prvPurchaseReturn->vendor_name }}</td>
                    <td>{{ $prvPurchaseReturn->product_name }}</td>
                    <td>{{ $prvPurchaseReturn->model_no }}</td>
                    <td>{{ $prvPurchaseReturn->qty }}</td>
                    <td align="right">{{ $prvPurchaseReturn->mrp_tk }}</td>
                    <td align="right">{{ $prvPurchaseReturn->cp_tk }}</td>
                    <td align="right">{{ $prvPurchaseReturn->discount_receipt }}</td>
                    <td align="right">{{ $prvPurchaseReturn->total_amount + $preTotalAmt}}</td>

                </tr>
                @php
                   
                   $preTotalAmt = $preTotalAmt + $prvPurchaseReturn->total_amount;
