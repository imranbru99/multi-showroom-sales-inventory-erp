...
<input type="hidden" name="id" value="{{ $loan->id }}">
    <div class="row">
        <div class="col-md-6">
            <div class="form-group {{ $errors->has('staff_id') ? ' has-danger' : '' }}">
                ...
                <select class="form-control chosen-select" name="staff_id" id="staff">
                    ...
                    <option value="{{ $staff->id }}" @if(@$loan->staff->id == $staff->id) selected @endif>{{ $staff->name }} ({{ $staff->code }})</option>
                    @endforeach
                </select>
        ...
        <div class="col-md-3">
            <div class="form-group {{ $errors->has('date') ? ' has-danger' : '' }}">
                ...
                <input type="text" class="form-control datepicker" name="date" value="{{ date('d-m-Y', strtotime($loan->date)) }}" required>
                @if ($errors->has('date'))
        ...
        <div class="col-md-3">
            <div class="form-group {{ $errors->has('type') ? ' has-danger' : '' }}">
                ...
                <select class="form-control" id="type" name="type">
                    ...
                    <option value="Advance" @if(@$loan->type == 'Advance') selected @endif>Advance</option>
                    <option value="Loan" @if(@$loan->type == 'Loan') selected @endif>Loan</option>
                </select>
                @if ($errors->has('type'))
        ...
        <div class="col-md-6">
            <div class="form-group {{ $errors->has('total_installment') ? ' has-danger' : '' }}">
                ...
                <input type="number" class="form-control" id="total_installment" name="total_installment" value="{{ $loan->total_installment }}">
    ...
    <div class="row mt-5 d-flex justify-content-center">
        <div class="form-group col-md-3">
            ...
            <input type="month" class="form-control mt-1" id="start_date" name="start_date" value="{{ date('Y-m', strtotime(@$loan->schedules[0]->date)) }}">
        </div>
