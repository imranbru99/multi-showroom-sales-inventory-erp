    ...
    <div class="row">
        <div class="col-md-4">
            <div class="form-group {{ $errors->has('payment_date') ? ' has-danger' : '' }}">
                ...
                <input type="text" class="form-control datepicker" name="payment_date" value="{{ date('d-m-Y', strtotime($loanPayment->payment_date)) }}" required>
        ...
        <div class="col-md-4">
            <div class="form-group {{ $errors->has('loan_no') ? ' has-danger' : '' }}">
                ...
                <select class="form-control chosen-select" name="loan_id" onchange="loanSchedules()" id="loan">
                    ...
                    <option value="{{ $loan->id }}" @if($loanPayment->loan_id == $loan->id) selected @endif>{{ $loan->loan_no }} ({{$loan->bank_name}})</option>
                    @endforeach
                </select>
            </div>
        </div>
        <div class="col-md-4">
            <div class="form-group {{ $errors->has('payment_amount') ? ' has-danger' : '' }}">
                ...
                <input type="text" class="form-control" name="payment_amount" id="payment_amount" value="{{ $loanPayment->paid_amount }}" readonly>
            ...
            <div class="col-md-6">
                <div class="table-responsive">
                    <table class="table table-bordered table-striped" id="schedule">
                        ...
                        <tbody>
                            @foreach($loanPayment->payments as $payment)
                                 <tr>
                                    <td>{{ $loop->iteration }}</td>
                                    <td><input type="text" class="form-control" name="schedule_date[]" value="{{ date('d-m-Y', strtotime($payment->payment_date)) }}" readonly></td>
                                    <td><input type="number" class="form-control text-right" id="schedule_amount_{{$payment->id}}" name="schedule_amount[]" value="{{ $payment->paid_amount }}" readonly></td>
                                    <td><input type="checkbox" class="form-control" id="check_{{$payment->id}}" name="schedule_id[]" value="{{$payment->id}}" onclick="checkValue({{$payment->id}});" checked></td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
