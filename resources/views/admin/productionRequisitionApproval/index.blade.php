                    ...
                    <table class="table table-bordered table-sm">
                        <thead>
                            <tr>
                                ...
                                <th width="50px">Date</th>
                                <th width="50px">Requisition No.</th>
                                <th width="350px">Requisition Name</th>
                                <th width="50px">Total Qty</th>
                                <th width="20px">Action</th>
                            </tr>
                        </thead>
        ...
        function showDetailRequisition(productionRequisitionId) {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('.dealerName').html(productionRequisition.staff.name);
                    $('.totalQty').val(productionRequisition.total_qty);
                    $('.productionRequisitionId').val(productionRequisition.id);

                    if (productionRequisition.approved_qty != null)
                    {
                        $('.totalApproveQty').val(productionRequisition.approved_qty);
                    }

                    if (productionRequisition.suggested != null)
                    {
                        $('.totalSuggestedQty').val(productionRequisition.suggested);
                    }
                    ...
                    for (var productionRequisitionInf of productionRequisitionInfo)
                    {
                        ...
                        $(".detailRequisitionProduct tbody").append(
                            '<tr class="requisitionProductRow" id="requisitionProductRow_'+productionRequisitionInf.id+'">' +
                                ...
                                '<td>'+
                                    '<input style="text-align: right;" class="form-control SuggestionQty SuggestionQty_'+productionRequisitionInf.id+'" oninput="rowSum()" type="number" name="SuggestionQty[]" value="'+productionRequisitionInf.suggested_qty+'" readonly>'+
                                '</td>'+
                                '<td>'+
                                    '<input style="text-align: right;" class="form-control approveQty approveQty_'+productionRequisitionInf.id+'" oninput="findTotalApproveAmount('+productionRequisitionInf.id+')" type="number" name="approveQty[]" value="'+approveQty+'">'+
                                '</td>'+
                            '</tr>'
                        );
                    }
