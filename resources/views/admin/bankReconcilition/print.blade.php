...
@section('content')
    <table id="report-header">
        <tr>
            @if ($fromDate == "" && $toDate == "")
                <td>Reconciliation Voucher List From</td>
            @else
                <td>Reconciliation Voucher List From {{ date('d-m-Y',strtotime($fromDate)) }} Month  Of {{ date('d-m-Y',strtotime($toDate)) }}</td>
            @endif
        </tr>
    </table>
        ...
        <tbody>            
            ...
            @foreach ($transactionLists as $transactionList)
                @php
                    ...
                    if(!@$accountHeadName->debitHeadname){
                        continue;
                    }
