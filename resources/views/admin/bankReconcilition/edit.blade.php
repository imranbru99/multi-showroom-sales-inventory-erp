        ...
        <div class="row">
            <div class="col-md-6">
                ...
                <div class="form-group">
                    <input type="text" class="form-control" id="showroomName" name="showroomName"
                        value="{{ $journalEntry->showroomName }}" readonly>
                    <input type="hidden" class="form-control" id="showroom" name="showroom"
                        value="{{ $journalEntry->showroom_id }}" readonly>
                    <input type="hidden" class="form-control" id="createdBy" name="createdBy"
                        value="{{ $journalEntry->created_by }}" readonly>
                </div>
            </div>
            ...
            <div class="col-md-3">
                <div class="form-group {{ $errors->has('voucharNo') ? ' has-danger' : '' }}">
                    ...
                    <input type="text" class="form-control" id="voucharNo" name="voucharNo"
                        value="{{ $journalEntry->voucher_no }}" placeholder="Enter Vouchar No" readonly>
            ...
            <div class="col-md-3">
                <div class="form-group {{ $errors->has('name') ? ' has-danger' : '' }}">
                    ...
                    <input type="text" class="form-control add_datepicker" name="transactionDate"
                        value="{{ $journalEntry->voucher_date }}" placeholder="Select Transaction Date" readonly>
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="form-group {{ $errors->has('remarks') ? ' has-danger' : '' }}">
                    ...
                    <textarea class="form-control remarks" id="remarks" name="remarks"
                        rows="2">{{ $journalEntry->narration }}</textarea>
                    ...
                    <table class="table table-bordered table-striped gridTable">
                        ...
                        <tbody id="tbody">
                            ...
                            @foreach ($journalEntries as $journalEntryInfo)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input style="text-align: right;" class="debit debit_{{ $i }}"
                                            id="debit" name="debit[]" oninput="findTotal()"
                                            value="{{ $journalEntryInfo->debit_amount }}">
                                    </td>

                                    <td>
                                        <input style="text-align: right;" class="credit credit_{{ $i }}"
                                            id="credit" name="credit[]" oninput="findTotal()"
                                            value="{{ $journalEntryInfo->credit_amount }}">
                                    </td>
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <td><input style="text-align: right" class="totalDebit" id="totalDebit"
                                        name="totalDebit" value="{{ $totalDebit }}"></td>
                                <td><input style="text-align: right" class="totalCredit" id="totalCredit"
                                        name="totalCredit" value="{{ $totalCredit }}"></td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
...
@section('custom-js')
    <script type="text/javascript">
        function addItem() {
            ...
            var total = parseFloat(rowcount) + 1;
            ...
            $(".gridTable tbody").append(
                ...
                '<td>' +
                '<input style="text-align: right;"  class="debit debit_' + total +
                '" id="debit" name="debit[]" oninput="findTotal()" value="0">' +
                '</td>' +

                '<td>' +
                '<input style="text-align: right;"  class="credit credit_' + total +
                '" id="credit" name="credit[]" oninput="findTotal()" value="0">' +
                '</td>' +
        ...
        function removeItem(i) {
            ...
            var remarks = "";
            var loop = $('.row-count').val();
            var debit = parseFloat($('.debit_' + i).val());
            var credit = parseFloat($('.credit_' + i).val());
    ...
    <script>
        $(".buttonAddEdit").click(function(e) {
            ...
            if (debit <= 0) {
                swal({
                    title: "Debit and Credit will be greater than 0",
                    type: "warning",
                    // timer: 1000,
                });
            }
            ...
            if (credit != debit) {
                swal({
                    title: "Credit and Debit have to same.",
                    type: "warning",
                    // timer: 1000,
                });
            }
